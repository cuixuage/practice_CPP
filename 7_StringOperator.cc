...
String & String::operator=(const String &rhs){	//assignment 赋值运算符重载(前提:左值对象已存在)
	if(this != &rhs){
		delete [] _pstr;
		_pstr=new char [strlen(rhs._pstr)+1];
		strcpy(_pstr,rhs._pstr);
...
const char & String::operator[](std::size_t index) const{           //const对象只能调用const成员函数;普通对象均能调用const,非const成员函数
	static char nullchar = '\0';
	if(index>=0 && index<strlen(_pstr)){
		return _pstr[index];
	}
...
/*
1.const关键字
1.1const放在函数前面,返回值类型是const; 
1.2const放在函数声明后面,则该函数不允许修改类的数据成员,不能通过此函数更改类的public,private,protected变量
1.3
...
此时,类对象的常量性决定调用哪个函数.非const类对象调用非const成员函数

2.static/const 关键字？
https://blog.csdn.net/tobefxz/article/details/14109697
2.1 初始化方式不同？直接初始化 / 构造函数的初始化列表
2.2 作用范围不同？ 所有对象 / 对象私有
2.3 "static const" 静态常量的写法
2.4 static全局变量只在当前源文件下有效; 全局变量在所有的源文件下有效。